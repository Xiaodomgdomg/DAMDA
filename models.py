...
class _netG(nn.Module):
    def __init__(self, opt, in_channels):
        ...
        self.main = nn.Sequential(
            nn.ConvTranspose2d(self.in_channels+self.nz, self.ngf*8, 2, 1, 0, bias=False),
...
class _netD(nn.Module):
    def __init__(self, opt, nclasses, ndomains):
        ...
        self.classifier_c = nn.Sequential(nn.Linear(self.ndf*2, nclasses)) # class
        self.classifier_d = nn.Sequential(nn.Linear(self.ndf*2, ndomains)) # domain            
        self.classifier_s = nn.Sequential(                                     # real/fake score
    ...
    def forward(self, input):       
        ...
        output_d = self.classifier_d(output.view(-1, self.ndf*2))
        output_s = self.classifier_s(output.view(-1, self.ndf*2))
        output_s = output_s.view(-1)
        return output_s, output_c, output_d
